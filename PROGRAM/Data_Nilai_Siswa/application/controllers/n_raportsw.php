...
Class n_raportsw extends CI_Controller {

    function __construct() {
        parent::__construct();
        //chekAksesModule();
        $this->load->library('ssp');
        // $this->load->model('Model_n_raportsw');
...
function LISTS(){
  // $data['dataNilai'] = $this->Model_n_raportsw->dataTableN2();
...
var_dump($id_user);
 ...
 $this->template->load('template', 'n_raportsw/list3',$data);
    }
        ...
        (
             ...
             "dataNilaiT" =>$this->db->query("SELECT *
              FROM tb_r_n_p_h join tb_r_n_p_d on tb_r_n_p_h.ID_P_H=tb_r_n_p_D.ID_P_H  JOIN tb_r_n_harian_k_h ON tb_r_n_p_h.NISN=tb_r_n_harian_k_h.NISN JOIN tb_r_harian_k_d on tb_r_harian_k_d.ID_Nilai_H_K=tb_r_n_harian_k_h.ID_Nilai_H_K join tb_m_siswa on tb_r_n_p_h.NISN=tb_m_siswa.NISN join tb_m_rombel on tb_r_n_p_h.ID_Rombel=tb_m_rombel.ID_Rombel join tbl_tahun_akademik on tb_r_n_p_h.id_tahun_akademik=tbl_tahun_akademik.id_tahun_akademik WHERE tb_r_n_harian_k_h.S_R=0 AND tb_r_n_p_h.S_R=0 group by tb_r_n_p_h.NISN ")->result(),
               ...
               // $data['dataNilaiT1'] = $this->Model_n_raportsw->dataTable();
       $this->template->load('template', 'n_raportsw/list',$data);
    }

        public function No_Induk($NISN)
    {
        $this->Model_n_raportsw->getIndukSiswa($NISN);
        
    }

    public function TB($NISN)
    {
        $this->Model_n_raportsw->getTB($NISN);
    ...
    {
        $this->Model_n_raportsw->getBB($NISN);
        
    }

       public function Penglihatan($NISN)
    {
        $this->Model_n_raportsw->getLihat($NISN);
        
    }

           public function Pendengaran($NISN)
    {
        $this->Model_n_raportsw->getDengar($NISN);
        
    }

               public function Gigi($NISN)
    {
        $this->Model_n_raportsw->getGigi($NISN);
        
    }

       public function NISN($NISN)
    {
        $this->Model_n_raportsw->getNISN1($NISN);
        
    }

           public function Nama_Rombel($NISN)
    {
        $this->Model_n_raportsw->getRombel($NISN);
        
    }
  
             public function tahun_akademik($NISN)
    {
        $this->Model_n_raportsw->getTahunAkademik($NISN);
        
    }

             public function Semester($NISN)
    {
        $this->Model_n_raportsw->getSemester($NISN);
        
    }

             public function Nilai_Pengetahuan($NISN)
    {
        $this->Model_n_raportsw->getNilaiP($NISN);
   ...
   public function pencarian_d($NISN){
    ...
    function add($ID_Legger) {
        ...
        $data=array(
            'dataSw'=>$this->Model_n_raportsw->getSiswa(),
            'dataMpl'=>$this->Model_n_raportsw->getCmbMapel(),
        ...
        // $data['dataSw'] = $this->Model_n_raportsw->getSw2();

        $data['hasil'] = $this->Model_n_raportsw->pencarian_d($NISN)->result_array();

$this->template->load('template', 'n_raportsw/add',$data);

//         if (isset($_POST['submit'])) {

//             $this->Model_n_raportsw->save();
...
$data['dataNilai2'] = $this->Model_n_raportsw->dataTableN();
$data['dataTB'] = $this->Model_n_raportsw->data2();

        $output = $this->load->view('n_raportsw/r_raport',$data,true);
...
"getrow3"=>$this->db->query("SELECT * FROM tb_r_n_p_h JOIN tb_m_tb_bb on tb_r_n_p_h.NISN=tb_m_tb_bb.NISN WHERE tb_r_n_p_h.NISN = '$id' group by tb_r_n_p_h.NISN")->result(),
        ...
        "dataCmbEks" => $this->Model_n_raportsw->getEkskul(),

        'ID_Raport'=>$this->Model_n_raportsw->getID(),
        );

        $NISN      = $this->uri->segment(3);
        // $ID_P_H      = $this->uri->segment(4);
        $data['n_raportsw'] = $this->db->get_where('tb_r_n_p_h',array('NISN'=>$NISN))->row_array();
        $this->template->load('template', 'n_raportsw/edit',$data);
...
function add2(){
     // if (isset($_POST['submit'])) {
        ...
        $data['n_raportsw'] = $this->db->get_where('tb_r_legger_h',array('ID_Legger'=>$id))->row_array();
...
// }
//         else {
//             $this->template->load('template', 'n_raportsw/list2',$data);
//         }
}
